...
class AnalyticalMethod(object):
    ...
    def feature_transform(self, X):
        ...
        f_transform = np.concatenate((X, bias), axis = 1)
    ...
    def compute_weights(self, X, y):
        ...
        self.W = np.linalg.matmul(np.linalg.matmul(np.linalg.pinv(np.linalg.matmul(X_T,X)), X_T), y)
    ...
    def predict(self, X):
        ...
        X_T = np.linalg.matrix_transpose(X)
        prediction = np.matmul(np.linalg.pinv(np.linalg.matmul(np.linalg.pinv(np.linalg.matmul(X_T,X)), X_T)), self.W)
